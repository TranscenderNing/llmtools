...
class NormalPeftNLGDataset(Dataset):
    def __init__(
        ...
        self.result = []
        self.cache_path = os.path.join(data_path, f"method.{method}.max_n_example.{self.max_n_example}.split.{self.data_split}.cache")
    ...
    def tokenize(self, data_item, id):
        ...
        if self.task == "e2e" or self.task == "ARC-Challenge":
            base_prompt = self.task_prompt_template % (data_item["context"])
            base_input = (
                base_prompt
                + self.trigger_tokens
