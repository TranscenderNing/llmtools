...
from transformers import (AutoModelForCausalLM, AutoTokenizer,
                          DataCollatorForSeq2Seq, GPT2Tokenizer, GPT2LMHeadModel)
...
def load_gpt_tokenizer(model_path, max_length=512):
...
def load_gpt_model(model_path, dtype, device, tokenizer_len, need_resize=False):
...
def get_module_hook(model, representation) -> nn.Module:
    ...
    parameter_name = f'transformer.h[{representation["layer"]}]'
...
# 创建一个函数来查找每一行第一个不是50256的元素的索引
def first_non_50256_index_in_rows(matrix):
    indices = []
    for row in matrix:
        # 创建一个掩码，表示元素是否不是50256
        mask = row != 50256
        # 使用torch.nonzero()找到非50256元素的索引
        non_50256_indices = torch.nonzero(mask).squeeze()
        # 如果有非50256的元素，记录第一个非50256元素的索引
        if non_50256_indices.numel() > 0:
            indices.append(non_50256_indices[0].item())
        else:
            # 如果没有非50256的元素，返回None
            indices.append(None)
    return indices
    ...
    with torch.no_grad():
        for _, inputs in enumerate(eval_iterator):
            ...
            for id, pred in zip(inputs["id"].tolist(), actual_preds):
                ...
                if task == "commonsense" or task == "ARC-Challenge" or True:
