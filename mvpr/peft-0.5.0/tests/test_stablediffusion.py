...
class StableDiffusionModelTester(TestCase, PeftCommonTester):
    ...
    def test_add_weighted_adapter_base_unchanged(
        ...
        self.assertTrue(
            asdict(text_encoder_adapter_config)
            == asdict(model.text_encoder.peft_config[text_encoder_adapter_name])
        )
        self.assertTrue(
            asdict(unet_adapter_config)
            == asdict(model.unet.peft_config[unet_adapter_name])
        )
