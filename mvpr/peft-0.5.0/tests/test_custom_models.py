...
TEST_CASES = [
    ...
    (
        "Embedding + transformers Conv1D 1",
        "EmbConv1D",
        LoraConfig,
        {"target_modules": ["conv1d"]},
    ),
    (
        "Embedding + transformers Conv1D 2",
        "EmbConv1D",
        LoraConfig,
        {"target_modules": ["emb"]},
    ),
    (
        "Embedding + transformers Conv1D 3",
        "EmbConv1D",
        LoraConfig,
    ...
    def test_only_params_are_updated(
        ...
        for name, param_before in params_before.items():
            ...
            if ("lora_" in name) or ("modules_to_save" in name):
                ...
                self.assertFalse(
                    torch.allclose(param_before, param_after, atol=tol, rtol=tol)
                )
            else:
                self.assertTrue(
                    torch.allclose(param_before, param_after, atol=tol, rtol=tol)
