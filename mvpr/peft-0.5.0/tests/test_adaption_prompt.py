...
class AdaptionPromptTester(TestCase, PeftCommonTester):
    ...
    def test_sequence_adapter_ops(self) -> None:
        ...
        self.assertFalse(
            torch.allclose(adapter_1_before.logits, adapter_1_after.logits)
        )
        ...
        self.assertFalse(
            torch.allclose(original_before.logits, default_after_set.logits)
        )
        self.assertFalse(
            torch.allclose(adapter_1_after.logits, default_after_set.logits)
        )
