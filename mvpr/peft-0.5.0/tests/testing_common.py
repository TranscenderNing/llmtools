...
from peft import (AdaLoraConfig, IA3Config, LoraConfig, PeftModel,
                  ...
                  PromptLearningConfig, PromptTuningConfig, get_peft_model,
...
class PeftCommonTester:
    ...
    def _test_save_pretrained(self, model_id, config_cls, config_kwargs):
        ...
        with tempfile.TemporaryDirectory() as tmp_dirname:
            ...
            self.assertTrue(
                os.path.exists(os.path.join(tmp_dirname, "adapter_config.json"))
            )
    ...
    def _test_save_pretrained_selected_adapters(
        ...
        with tempfile.TemporaryDirectory() as tmp_dirname:
            ...
            self.assertTrue(
                "new_adapter" not in model_from_pretrained.peft_config.keys()
            )
    ...
    def _test_merge_layers(self, model_id, config_cls, config_kwargs):
        ...
        self.assertTrue(
            torch.allclose(logits_unmerged, logits_merged, atol=1e-4, rtol=1e-4)
        )
        ...
        self.assertFalse(
            torch.allclose(logits_merged, logits_transformers, atol=1e-10, rtol=1e-10)
        )
        ...
        self.assertTrue(
            torch.allclose(
                logits_merged, logits_merged_from_pretrained, atol=1e-4, rtol=1e-4
            )
        )
    ...
    def _test_inference_safetensors(self, model_id, config_cls, config_kwargs):
        ...
        with tempfile.TemporaryDirectory() as tmp_dirname:
            ...
            self.assertTrue(
                torch.allclose(logits, logits_from_pretrained, atol=1e-4, rtol=1e-4)
            )
    ...
    def _test_unload_adapter(self, model_id, config_cls, config_kwargs):
        ...
        if config.peft_type not in ("LORA", "ADALORA"):
            ...
            self.assertFalse(
                torch.allclose(logits_with_lora, logits_unload, atol=1e-10, rtol=1e-10)
            )
            self.assertTrue(
                torch.allclose(logits_transformers, logits_unload, atol=1e-4, rtol=1e-4)
            )
    ...
    def _test_disable_adapter(self, model_id, config_cls, config_kwargs):
        ...
        if isinstance(peft_model, StableDiffusionPipeline):
            ...
            self.assertTrue(
                (output_before != output_peft_disabled).float().mean() < 1e-4
            )
        else:
