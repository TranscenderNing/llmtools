...
from ..utils import (TRANSFORMERS_MODELS_TO_ADALORA_TARGET_MODULES_MAPPING,
                     PeftType, _freeze_adapter, _get_submodules,
                     get_auto_gptq_quant_linear, get_quantization_config,
                     transpose)
from .lora import LoraConfig, LoraLayer, LoraModel
...
class AdaLoraLayer(LoraLayer):
    ...
    def update_layer(
        ...
        self.lora_A.update(
            nn.ParameterDict(
                {adapter_name: nn.Parameter(torch.randn(r, self.in_features))}
            )
        )
        # Singular values
        self.lora_E.update(
            nn.ParameterDict({adapter_name: nn.Parameter(torch.randn(r, 1))})
        )
        # Left singular vectors
        self.lora_B.update(
            nn.ParameterDict(
                {adapter_name: nn.Parameter(torch.randn(self.out_features, r))}
            )
        )
        # The current rank
        self.ranknum.update(
            nn.ParameterDict(
                {adapter_name: nn.Parameter(torch.randn(1), requires_grad=False)}
            )
        )
