...
class LoraLayer(BaseTunerLayer):
    ...
    def update_layer(self, adapter_name, r, lora_alpha, lora_dropout, mode, init_lora_weights):
        ...
        for i, rank in enumerate(r):
            ...
            if rank > 0:
                self.lora_A.append(nn.Parameter(torch.randn(rank, self.in_features)))
                self.mix.append(nn.Parameter(torch.randn(rank)))
                self.lora_B.append(nn.Parameter(torch.randn((self.out_features, rank))))
                self.scaling.append(lora_alpha[i] / rank)
                # if i == 0:
                self.lora_A[i].data = u[i*(self.in_features):(i+1)*(self.in_features), :rank].T.contiguous().to(weight_dtype)
                self.mix[i].data = s[:rank].to(weight_dtype)
                self.lora_B[i].data = vt[:rank, i*(self.in_features):(i+1)*(self.in_features)].T.contiguous().to(weight_dtype)
                merge = (self.lora_B[i] * self.mix[i]) @ self.lora_A[i]
                self.weight.data = (self.weight - merge * self.scaling[i]).to(weight_dtype)
                # else:
                #     self.lora_A[i].data = self.lora_A[i-1].data.clone()
                #     self.mix[i].data = self.mix[i-1].data.clone()
                #     self.lora_B[i].data = self.lora_B[i-1].data.clone()
                print(self.weight.dtype, self.lora_A[i].dtype, self.lora_B[i].dtype, self.mix[i].dtype)
                
                
                
                
                # self.lora_A.append(nn.Linear(self.in_features, rank, bias=False))
                # self.mix.append(nn.Linear(rank, rank, bias=False))
                ...
                # invalid
                # torch.nn.init.orthogonal_(self.lora_A[i].weight)
                # nn.init.zeros_(self.mix[i].weight)
                # torch.nn.init.orthogonal_(self.lora_B[i].weight)
                
                # nn.init.kaiming_uniform_(self.lora_A[i].weight, a=math.sqrt(5))
                # nn.init.kaiming_uniform_(self.mix[i].weight, a=math.sqrt(5))
                # nn.init.zeros_(self.lora_B[i].weight)
    ...
    **Attributes**:
    ...
    def _mark_only_adapters_as_trainable(self) -> None:
        ...
        for n, p in self.model.named_parameters():
            if "lora_" not in n and "mix" not in n:
...
class Linear(nn.Linear, LoraLayer):
    ...
    def forward(self, x: torch.Tensor):
        ...
        if self.disable_adapters:
            ...
            x = x.to(self.lora_A[0].dtype)
            temp = []
            for i, rank in enumerate(self.r[self.active_adapter]):
                if rank > 0:
                    print(x[:,:,i*(self.in_features):(i+1)*(self.in_features)].shape)
                    print(self.lora_B[i].shape)
                    print(self.mix[i].shape)
                    print(self.lora_A[i].shape)
                    temp.append(
                        F.linear(
                            self.lora_dropout[i](x[:,:,i*(self.in_features):(i+1)*(self.in_features)]),
                            (self.lora_B[i] * self.mix[i]) @ self.lora_A[i],
                        ) * self.scaling[i]
                    )
                    ...
                    # )
            for item in temp:
                print('concat shape', item.shape)
            print('result shape', result.shape)
            torch.concat(temp, dim=-1)
