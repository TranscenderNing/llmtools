...
from transformers import (AutoTokenizer,
                          DataCollatorForSeq2Seq, T5ForConditionalGeneration, T5Tokenizer)
...
def get_args():
    ...
    parser.add_argument("-eval_batch_size", "--eval_batch_size", type=int, default=1)
    parser.add_argument(
    ...
    parser.add_argument("-test_split", "--test_split", type=str, default="dev")
...
def get_module_hook(model, representation) -> nn.Module:
    ...
    parameter_name = f'decoder.block[{representation["layer"]}]'
...
def scatter_neurons(
    ...
    print('tensor input', tensor_input.shape)
    print("unit_locations_as_list", unit_locations_as_list)
    unit_locations = torch.tensor(unit_locations_as_list, device=tensor_input.device)
...
def compute_metrics(
    ...
    with torch.no_grad():
        for _, inputs in enumerate(eval_iterator):
            ...
            print("intervention_locations 1", intervention_locations.shape)
            # # get left padding count, [batch_size], and add to locations
            # left_padding = (inputs["input_ids"] == 863).nonzero(
            #     as_tuple=True
            # )[1]
            ...
            print("shape", intervention_locations.repeat_interleave(
                num_beams, dim=1
            ).shape)
